      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <template v-if="hasPerm('flcFlowinstance:cancel') && record.isFinish==0?true:false">
              <a-popconfirm placement="topRight" title="确认撤销？" @confirm="() => flcFlowinstanceCancel(record)">
                <a>撤销</a>
              </a-popconfirm>
          </template>
          <a-divider type="vertical" v-if="hasPerm('flcFlowinstance:cancel') && record.isFinish==0?true:false" />
