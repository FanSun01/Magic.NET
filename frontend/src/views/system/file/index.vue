      ...
      <s-table ref="table" :columns="columns" :data="loadData" :alert="true" :rowKey="(record) => record.id"
        ...
        <template slot="operator" v-if="hasPerm('sysFileInfo:upload')">
          <a-upload :customRequest="customRequest" :multiple="true" :showUploadList="false" name="file"
            ...
            <a-progress v-if="isUpload" :percent="percent" />
          </a-upload>
        </template>
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-popconfirm v-if="hasPerm('sysFileInfo:delete')" placement="topRight" title="确认删除？"
            @confirm="() => sysFileInfoDelete(record)">
            <a>删除</a>
          </a-popconfirm>
          <a-divider type="vertical"
            v-if="(hasPerm('sysFileInfo:preview') & record.fileSuffix === 'png' || record.fileSuffix === 'jpeg' || record.fileSuffix === 'jpg' || record.fileSuffix === 'gif' || record.fileSuffix === 'tif' || record.fileSuffix === 'bmp') & hasPerm('sysFileInfo:delete')" />
          ...
          <a-divider type="vertical"
            v-if="(hasPerm('sysFileInfo:preview') & record.fileSuffix === 'doc' || record.fileSuffix === 'docx' || record.fileSuffix === 'xls' || record.fileSuffix === 'xlsx') & hasPerm('sysFileInfo:delete')" />
